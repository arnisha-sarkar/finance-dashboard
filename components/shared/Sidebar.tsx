...
const Sidebar = ({ isOpen, setIsOpen }: SidebarProps) => {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed inset-y-0 left-0 z-50 w-72 bg-card-bg border-r border-border-custom flex flex-col h-screen transition-all duration-300 ease-in-out
          lg:static lg:translate-x-0
          ${isOpen ? "translate-x-0" : "-translate-x-full"}
        `}
      >
        ...
        <div className="p-8 flex items-center justify-between">
          ...
          <button
            onClick={() => setIsOpen(false)}
            className="lg:hidden p-2 text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-full transition-colors"
          >
            <X size={20} />
          </button>
        </div>
          ...
          {menuItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group ${
                  isActive
                    ? "bg-blue-600 text-white font-bold shadow-lg shadow-blue-500/20 scale-[1.02]"
                    : "text-slate-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 font-medium"
                }`}
              >
                <Icon
                  size={18}
        ...
        {/* User Profile Section */}
        <div className="p-4 mt-auto">
          {/* প্রোফাইল কার্ডটি সব মোডেই ডার্ক রাখা ভালো, তবে বর্ডারটি ডাইনামিক করেছি */}
          <div className="bg-[#0f172a] border border-white/5 rounded-[24px] p-6 text-white relative overflow-hidden group shadow-2xl">
            {/* Decoration Glow */}
            <div className="absolute -right-4 -top-4 w-16 h-16 bg-blue-500/10 rounded-full blur-xl group-hover:bg-blue-500/20 transition-all duration-500"></div>
