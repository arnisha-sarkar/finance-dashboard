  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 bg-background text-foreground transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 py-3 rounded-2xl font-bold text-sm transition-all shadow-lg shadow-blue-200 dark:shadow-none active:scale-95 group">
          <Plus
            size={18}
            className="group-hover:rotate-90 transition-transform"
          />
      ...
      {/* Main Net Worth Card (এটি সবসময় ডার্ক থাকে, তাই বর্ডার অ্যাডজাস্ট করা হয়েছে) */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {accountsData.map((account: Account) => {
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between mb-8">
                {/* Icon Container - কালারগুলো আপনার mockData থেকে আসছে, তাই এখানে shadow অ্যাডজাস্ট করা হয়েছে */}
                <div
                  className={`p-4 rounded-2xl ${account.color} text-white shadow-lg transform group-hover:scale-110 transition-transform`}
                >
                  <Icon size={24} />
